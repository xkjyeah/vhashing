...
int main() {
  ...
  // erase from blockmap
  std::vector<int3> keys_to_erase;
  {

    // purge half the keys...
    for (int i=0; i<keys.size(); i+=2) {
      keys_to_erase.push_back(keys[i]);
    }

    thrust::device_vector<int3> dkeys_to_erase(keys_to_erase);

    int numJobs = keys_to_erase.size();
    int tpb = 16;
    int numBlocks = (numJobs + (tpb-1)) / tpb;
    erase_kernel<<<numBlocks, tpb>>>(
        (&dkeys_to_erase[0]).get(),
        keys_to_erase.size(),
        blocks);
  }
  ...
  vhashing::HashTable<int3, VoxelBlock, BlockHasher, BlockEqual, vhashing::std_memspace>
	...
	for (int i=0; i<keys_to_erase.size(); i++) {
		int3 key = keys_to_erase[i];
		VoxelBlock &value = values[i];

		if (bmh.find(key) != bmh.end()) {
			printf("%d\n", key.x, key.y, key.z);
      throw 1;
		}
	}
